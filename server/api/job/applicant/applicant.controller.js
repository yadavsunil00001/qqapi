...
export function update(req,res) {
  db.Applicant.getFullDetails(db,req.params.applicantId).then(existingApplicant => {
    ...
    return Promise.all([reqPr]).then(bodyPrRe => {
      ...
      if(files){
        var file = files.fileUpload;
        let fileExtension = file.name.split(".").pop().toLowerCase(); // Extension
        let allowedExtType = ['doc', 'docx', 'pdf', 'rtf', 'txt'];
        if (allowedExtType.indexOf(fileExtension.toLowerCase()) === -1) {
      ...
      if(!!editedApplicant.degree_id  && _.get(existingApplicant.Education[0], 'id') !== editedApplicant.degree_id ) {
        promises.push(db.Education.findById(_.get(existingApplicant.Education[0], 'id'))
      ...
      if (!!editedApplicant.salary
        && _.get(existingApplicant.Experience, 'salary') !== editedApplicant.salary) {
        experienceToSave.salary = editedApplicant.salary;
      }

      if (!!editedApplicant.region_id &&
        _.get(existingApplicant.Experience, 'region_id') !== editedApplicant.region_id) {
        experienceToSave.region_id = editedApplicant.region_id;
      }

      if (!!editedApplicant.employer_id &&
        _.get(existingApplicant.Experience, 'employer_id') !== editedApplicant.employer_id) {
        experienceToSave.employer_id = editedApplicant.employer_id;
      }

      if (!!editedApplicant.designation_id &&
        _.get(existingApplicant.Experience, 'designation_id') !== editedApplicant.designation_id) {
